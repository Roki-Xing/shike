...
fun calendarExecutionResult(): ExecutionResult =
    ExecutionResult(
        "日历",
        "待用户保存",
        "已打开日历草稿，等你在系统日历里保存；已打开系统日历新增页，尚未确认保存；返回拾刻后请标记是否保存。",
    )
...
fun mapExecutionResult(item: ShikeItem? = null): ExecutionResult =
    if (item != null && isCampusRoomCode(item.location)) {
        ExecutionResult("地点", "已复制", "已复制地点：${item.location}；校内教室代码不默认打开公网地图。")
    } else {
