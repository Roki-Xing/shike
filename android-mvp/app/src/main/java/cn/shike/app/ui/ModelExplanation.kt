...
fun modelExplanation(item: ShikeItem): String {
    val backendExplanation = item.rawText.substringAfter("后端 /v1/analyze：", missingDelimiterValue = "")
        .lineSequence()
        .firstOrNull()
        ?.takeIf { it.isNotBlank() }
    if (backendExplanation != null) {
        return backendExplanation
    }

    val reason = when {
        "后端不可用" in item.rawText -> "后端不可用，已使用本地规则保留行动卡，用户确认后再执行系统动作。"
