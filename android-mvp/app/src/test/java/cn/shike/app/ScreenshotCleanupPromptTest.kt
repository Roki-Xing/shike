...
class ScreenshotCleanupPromptTest {
    ...
    fun cleanupStatusLabel_usesUserFacingChineseCopy() {
        ...
        assertEquals("当前来源不支持直接移入回收站", cleanupStatusLabel(ImageCleanupStatus.NOT_SUPPORTED))
        ...
        assertEquals("正在等待系统确认", cleanupStatusLabel(ImageCleanupStatus.DELETE_REQUESTED))
        assertEquals("已移入系统回收站", cleanupStatusLabel(ImageCleanupStatus.DELETED))
