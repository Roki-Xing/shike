...
- Android 本地单元测试守卫已通过：`python3 shike/validation/validate_android_unit_tests.py` 输出 `ANDROID_UNIT_TEST_METRIC 61/61`；最近一次 `gradle --no-daemon :app:testDebugUnitTest` 通过。
...
| 后端解析 | 启动 FastAPI 后点“后端解析课程”或“后端解析活动” | Android 请求 `http://10.0.2.2:8000/v1/analyze` 并用响应更新行动卡 |
...
| 后端结果仍像样例 | 检查“后端解析课程/活动”前是否已编辑 OCR 文本草稿；空草稿会回退当前行动卡文本 |
