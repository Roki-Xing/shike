...
| 应用价值 | `validation/regression-cases.json`、`backend/shike_backend/eval/run_model_eval.py`、`android-mvp/app/src/main/java/cn/shike/app/data/InboxEntities.kt` | 作业/会议、面试/出行扩展补录可按 `materials/device-demo-checklist.md` 追加；真实访谈和问卷仍为 `待采集` | `docs/user-research-plan.md`、`docs/user-interview-summary.md`、`materials/evidence/scoring-evidence-map.md`、`USER_RESEARCH_EVIDENCE_METRIC 8/8`、`materials/evidence/requirement-matrix.md`、`REQUIREMENT_MATRIX_METRIC 9/9`、`MODEL_EVAL_METRIC 110/110` |
| 完成度 | Android APK、FastAPI 后端、SQLite 收件箱、执行层守卫、Android 图片预处理守卫、截图语义样例守卫、后端审计日志守卫、live-smoke 脱敏证据守卫、云真机 prep helper、公网后端预检、release handoff runner | 本地/USB 真机六段主线录屏；云真机 strict 录屏收齐前不宣称发布完成 | `LANDING_RELEASE_CANDIDATE_METRIC 63/63`、`BACKEND_AUDIT_LOG_METRIC 8/8`、`ANDROID_IMAGE_PREPROCESS_METRIC 15/15`、`IMAGE_SEMANTIC_CASES_METRIC 9/9`、`LIVE_SMOKE_EVIDENCE_METRIC 7/7`、`DELIVERABLES_METRIC 10/10`、`CLOUD_DEVICE_PREP_METRIC 5/5`、`CLOUD_BACKEND_PREFLIGHT_METRIC`、`CLOUD_DEVICE_PACKAGE_METRIC 30/30`、`RELEASE_HANDOFF_CHECKS_METRIC 24/24`、expected `LANDING_RELEASE_CANDIDATE_STRICT_EVIDENCE 3/7` |
| 大模型应用能力 | `contracts/model-adapter.md`、`contracts/model-output.schema.json`、`backend/shike_backend/adapters/bluelm_adapter.py` | BlueLM 在线 smoke 仅展示脱敏日志，不展示 AppKEY、签名或完整 OCR 原文 | `BLUELM_ADAPTER_METRIC 8/8`、`MODEL_CONTRACT_STRICT_METRIC 10/10`、`materials/evidence/cloud-device/backend-redacted-access-log.txt`、`PASS secret_hygiene` |

`materials/evidence/release-evidence-index.md` 是提交前的总证据入口；`validation/traceability.md` 的 SHIKE-070 行负责把提交材料、演示验收、云真机证据包、strict 阻断报告和 `DELIVERABLES_METRIC 10/10` 串到同一条可复核链路。`validation/traceability.md` 的 SHIKE-075 行负责把应用价值研究计划、访谈摘要模板、评分证据映射和 `USER_RESEARCH_EVIDENCE_METRIC 8/8` 串到同一条可复核链路，并明确真实访谈结果仍为 `待采集`，不得编造。真实云真机 9 段 MP4 与填写后的测试报告未收齐前，`LANDING_RELEASE_CANDIDATE_STRICT_EVIDENCE 3/7` 必须保持为外部证据阻断状态。云真机录制前还必须完成 `cloud-device-test-report.md` 的 `Pre-recording Evidence Gate`：确认 `/mnt/c/Users/Xing/Desktop/1. 当前仓库总体判断.md` 仍是桌面指导源，`materials/evidence/requirement-matrix.md` 仍通过 `REQUIREMENT_MATRIX_METRIC 9/9`，all 9 real cloud-device MP4 files 已进入证据包，且 no placeholder fields remain after capture。
