...
| Screenshot assist | `python3 shike/validation/validate_screenshot_assist.py` | `SCREENSHOT_ASSIST_METRIC 17/17`; guards the opt-in recent screenshot helper, restart-persistent switch, clear-cache reset, 30-second MediaStore lookback, notification candidate metadata preservation, notification confirmation, repeated-candidate notification dedupe shared by foreground and service observers, no overlay/accessibility main path, display-name digesting, and screenshot detection that requires filename or path screenshot signals instead of screen-size-only matches |
...
| Android unit tests | `python3 shike/validation/validate_android_unit_tests.py` | `ANDROID_UNIT_TEST_METRIC 90/90`; latest `gradle --no-daemon :app:testDebugUnitTest` reports 45 suites, 167 tests, 0 failures, 0 errors |
